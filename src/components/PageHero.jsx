...
export default function PageHero({ title, subtitle, image }) {
  return (
    <section className="relative flex min-h-[50vh] items-end overflow-hidden rounded-b-[3rem] bg-navy">
      ...
      >
        <h1 className="font-heading text-5xl text-sand sm:text-7xl lg:text-[5.25rem]">{title}</h1>
        <p className="mt-4 max-w-2xl text-lg text-sand/86 sm:text-xl lg:text-2xl">{subtitle}</p>
      </motion.div>
    </section>
  )
}
